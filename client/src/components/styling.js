...
export const StyledInput = styled(TextField)({
    marginBottom: '30px', '& .MuiOutlinedInput-root': {
        background: 'white', '&:hover .MuiOutlinedInput-notchedOutline': {
            borderColor: 'white',
...
export function ArticleContainer({link, children}) {
    ...
    if (link) {
        ...
        return (<Link style={Object.assign({}, style, linkStyle)} to={link}>
            {children}
        </Link>);
    } else {
...
export function AnyImage({width, url, alt}) {
    ...
    return (<img style={style} alt={alt ?? "image"} src={url ?? placeh}></img>);
}
